...
export const updateSong = createAsyncThunk(
  'songs/update',
  async ({ songId, songData }, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
      return await songService.updateSong(songId, songData, token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const songSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //     if (songIndex !== -1) {
    //       // Create a new song object with updated data
    //       const updatedSong = {
    //         ...state.songs[songIndex],
    //         ...action.payload,
    //       };
    //       // Replace the old song with the updated song in the state
